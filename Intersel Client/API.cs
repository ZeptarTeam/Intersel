...
namespace InterselClient_API
{
    ...
    internal class OnProgramStart
    {
        ...
        public static void Initialize(string name, string aid, string secret, string version)
        {
            ...
            {
                MessageBox.Show("Invalid application information!", Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    ...
                    switch (response[2])
                    {
                        case "success":
                            ...
                            else
                            {
                                ...
                                {
                                    MessageBox.Show($"Update {ApplicationSettings.Version} available, download from the discord server!", Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
                                    Process.GetCurrentProcess().Kill();
                                }
                        ...
                        case "binderror":
                            ...
                            return;
                        case "banned":
                            MessageBox.Show("This application has been banned for violating the TOS" + Environment.NewLine + "Contact us at support@auth.gg", Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
                            Process.GetCurrentProcess().Kill();
                            return;
                    }
                    Security.End();
    ...
    internal class API
    {
        ...
        public static bool AIORegister(string AIO)
        {
            ...
            if (string.IsNullOrWhiteSpace(AIO))
            {
                MessageBox.Show("Invalid registering information!", ApplicationSettings.Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
                Security.End();
                return false;
            }
            string[] response = new string[] { };
                ...
                try
                {
                    ...
                    switch (response[2])
                    {
                        case "success":
                            ...
                            return true;
                        case "invalid_details":
                            MessageBox.Show("Sorry, your username/password does not match!", ApplicationSettings.Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
                            Security.End();
                            return false;
                        case "time_expired":
                            MessageBox.Show("Your subscription has expired!", ApplicationSettings.Name, MessageBoxButtons.OK, MessageBoxIcon.Warning);
